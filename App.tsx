...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (step) {
      case 'source':
         return (
            <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-32 items-center animate-fade-in-up py-10 lg:py-0">
                ...
                <div className="flex flex-col text-center lg:text-left">
                    ...
                    <h2 className="text-6xl xl:text-8xl font-bold text-white mb-8 tracking-tighter leading-[0.9]">
                        Hei<br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand to-purple-500">wei.</span>
                    </h2>
                    <p className="text-zinc-400 text-xl lg:text-2xl font-light mb-12 max-w-lg mx-auto lg:mx-0 leading-relaxed">
                ...
                <div className="flex flex-col gap-6 w-full">
                    <div className="p-1.5 rounded-[2.5rem] bg-gradient-to-br from-white/10 via-brand/20 to-transparent">
                        <button 
                          ...
                          className="w-full group bg-zinc-950 p-10 rounded-[2.2rem] flex items-center justify-between hover:bg-zinc-900 transition-all duration-300 relative overflow-hidden"
                        >
                            ...
                            <div className="flex items-center gap-8 relative z-10">
                                <div className="w-20 h-20 bg-brand/10 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-brand/20">
                                  <CameraIcon className="w-8 h-8 text-brand"/>
                                </div>
                                <div className="text-left">
                                    <span className="block text-3xl font-bold text-white group-hover:text-brand transition-colors mb-1">Live Camera</span>
                                    <span className="block text-sm text-zinc-500 uppercase tracking-widest font-medium">Auto-Capture</span>
                                </div>
                            </div>
                            <span className="text-zinc-800 font-light text-5xl group-hover:translate-x-2 transition-transform relative z-10">→</span>
                        </button>
                    </div>

                    <button 
                      onClick={() => { setCaptureMode('upload'); setStep('capture'); }} 
                      className="w-full group bg-white/[0.03] p-10 rounded-[2.5rem] flex items-center justify-between hover:bg-white/[0.06] transition-all duration-300 border border-white/5"
                    >
                         <div className="flex items-center gap-8">
                            <div className="w-20 h-20 bg-white/[0.05] rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                              <UploadIcon className="w-8 h-8 text-zinc-400"/>
                            </div>
                            <div className="text-left">
                                <span className="block text-3xl font-bold text-white">Upload File</span>
                                <span className="block text-sm text-zinc-500 uppercase tracking-widest font-medium">Import Image</span>
                            </div>
                        </div>
                        <span className="text-zinc-700 font-light text-5xl group-hover:translate-x-2 transition-transform">→</span>
                    </button>
    ...
    <div className="min-h-screen flex flex-col items-center py-6 lg:py-12 px-4 lg:px-8 overflow-x-hidden selection:bg-brand/30">
      <main className="w-full max-w-[1400px]">
        ...
        <div className={`
            ...
            ${step === 'source' ? 'p-8 lg:p-24' : 'p-6 lg:p-12'}
        `}>
            {renderContent()}
        </div>
      </main>
